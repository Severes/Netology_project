...
# чтобы прочитать файл частично, мы можем использовать метод .readline
with open("props3.txt") as f3:
    ...
    # пустая строка появлется потому, что в файле props3 в конце каждой строки стоит перенос строки
    # питон и его печатет вместе с строкой, но прои помощи "END" мы можем ему сказть, чтобы он его не печатал
    print("-----")
